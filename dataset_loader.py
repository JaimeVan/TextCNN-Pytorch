...
def pad_list(input_list, target_length=50, pad_value=0):
...
class myWordVocabModel():
    def __init__(self, all_text, vector_size=1000, window=5, min_count=1, workers=4):

        self.tokens = []
        for text in all_text:
            self.tokens.append([token for token in jieba.cut(text)])

        self.word2vecModel = Word2Vec(sentences=self.tokens, vector_size=vector_size, window=5, min_count=1, workers=4)
    ...
    def text2vector(self, text):
        tokens = [token for token in jieba.cut(text) if re.search(cn_reg, token)]
        vectors = []
        for token in tokens:
            if token in self.word2vecModel.wv:
                vectors.append(self.word2vecModel.wv[token])  
            else:
                zero_vector = np.zeros(self.word2vecModel.vector_size)
    ...
    def get_token_indices(self, texts):
        ...
        for row in tokens:
            ...
            id_ = pad_list(id_, target_length=30)
            indices.append(id_)
...
def get_text_dataloader(csv_file, label_dict,vector_size=1000, test_ratio=0.3, batch_size=32, window=5, min_count=1, workers=4):
